<x-admin-layout>
    <x-slot name="header">
        Add Episode to Season {{ $season->season_number }} - {{ $content->title }}
    </x-slot>
    ...
    <div class="bg-white dark:bg-zinc-900 rounded-xl shadow-sm border border-gray-100 dark:border-zinc-800 overflow-hidden max-w-2xl">
        <form action="{{ route('admin.content.seasons.episodes.store', [$content, $season]) }}" method="POST" enctype="multipart/form-data" class="p-6">
            @csrf
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <input type="number" min="1" name="episode_number" id="episode_number" value="{{ old('episode_number', $nextEpisodeNumber) }}" required
                ...
                <div class="pt-2">
                    <label class="flex items-center">
                        <input type="checkbox" name="is_published" value="1" {{ old('is_published', true) ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500 dark:bg-zinc-900 dark:border-zinc-700">
