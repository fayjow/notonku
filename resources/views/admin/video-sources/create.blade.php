<x-admin-layout>
    ...
    <div class="mb-6">
        <a href="{{ route('admin.video-sources.index', ['sourceable_type' => $sourceable_type, 'sourceable_id' => $sourceable_id]) }}" class="text-sm text-indigo-600 dark:text-indigo-400 hover:text-indigo-900 dark:hover:text-indigo-300 flex items-center gap-1">
    ...
    <div class="bg-white dark:bg-zinc-900 rounded-xl shadow-sm border border-gray-100 dark:border-zinc-800 overflow-hidden max-w-2xl">
        <form action="{{ route('admin.video-sources.store') }}" method="POST" class="p-6" x-data="{ provider: '{{ old('provider', 'mp4') }}' }">
            @csrf
            
            <input type="hidden" name="sourceable_type" value="{{ $sourceable_type }}">
            <input type="hidden" name="sourceable_id" value="{{ $sourceable_id }}">
            ...
            <div class="space-y-6">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="server_name" id="server_name" value="{{ old('server_name') }}" required placeholder="e.g. Server 1, VIP Server"
                               class="mt-1 block w-full rounded-md border-gray-300 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                        @error('server_name')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="quality" id="quality" value="{{ old('quality') }}" placeholder="e.g. 1080p, 720p, 4K"
                               class="mt-1 block w-full rounded-md border-gray-300 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                        @error('quality')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                ...
                <div>
                    ...
                    <input type="text" name="language" id="language" value="{{ old('language') }}" placeholder="e.g. English Sub, Indonesia Sub, Dubbed"
                           class="mt-1 block w-full rounded-md border-gray-300 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                    @error('language')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                ...
                <div class="pt-2 flex gap-6">
                    <label class="flex items-center">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', true) ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500 dark:bg-zinc-900 dark:border-zinc-700">
                    ...
                    <label class="flex items-center" :class="{ 'opacity-50': provider !== 'mp4' }">
                        <input type="checkbox" name="is_downloadable" value="1" x-bind:disabled="provider !== 'mp4'" {{ old('is_downloadable') ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500 dark:bg-zinc-900 dark:border-zinc-700 disabled:opacity-50">
                    ...
                    <label class="flex items-center" :class="{ 'opacity-50': provider !== 'embed' }">
                        <input type="checkbox" name="supports_autoplay" value="1" x-bind:disabled="provider !== 'embed'" {{ old('supports_autoplay', false) ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500 dark:bg-zinc-900 dark:border-zinc-700 disabled:opacity-50">
